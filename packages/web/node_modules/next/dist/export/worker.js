"use strict";var _mkdirp=_interopRequireDefault(require("mkdirp"));var _util=require("util");var _path=require("path");var _render=require("next-server/dist/server/render");var _fs=require("fs");var _asyncSema=require("async-sema");var _amphtmlValidator=_interopRequireDefault(require("amphtml-validator"));var _loadComponents=require("next-server/dist/server/load-components");var _nextPageConfig=require("../build/babel/plugins/next-page-config");var _isDynamic=require("next-server/dist/lib/router/utils/is-dynamic");var _routeMatcher=require("next-server/dist/lib/router/utils/route-matcher");var _routeRegex=require("next-server/dist/lib/router/utils/route-regex");function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj};}const envConfig=require('next-server/config');const mkdirp=(0,_util.promisify)(_mkdirp.default);const writeFileP=(0,_util.promisify)(_fs.writeFile);const readFileP=(0,_util.promisify)(_fs.readFile);const accessP=(0,_util.promisify)(_fs.access);global.__NEXT_DATA__={nextExport:true};process.on('message',async({distDir,buildId,exportPaths,exportPathMap,outDir,renderOpts,serverRuntimeConfig,concurrency,subFolders,serverless})=>{const sema=new _asyncSema.Sema(concurrency,{capacity:exportPaths.length});try{const work=async path=>{await sema.acquire();const ampPath=`${path==='/'?'/index':path}.amp`;const{page}=exportPathMap[path];let{query={}}=exportPathMap[path];// Check if the page is a specified dynamic route
