...
const renderOpts={dir,buildId,nextExport:true,assetPrefix:nextConfig.assetPrefix.replace(/\/$/,''),distDir,dev:false,staticMarkup:false,hotReloader:null,canonicalBase:nextConfig.amp&&nextConfig.amp.canonicalBase||''};const{serverRuntimeConfig,publicRuntimeConfig}=nextConfig;if(Object.keys(publicRuntimeConfig).length>0){renderOpts.runtimeConfig=publicRuntimeConfig;}// We need this for server rendering the Link component.
global.__NEXT_DATA__={nextExport:true};log(`  launching ${threads} threads with concurrency of ${concurrency} per thread`);const exportPathMap=await nextConfig.exportPathMap(defaultPathMap,{dev:false,dir,outDir,distDir,buildId});exportPathMap['/404.html']=exportPathMap['/404.html']||{page:'/_error'};const exportPaths=Object.keys(exportPathMap);const progress=!options.silent&&(0,_ttyAwareProgress.default)(exportPaths.length);const chunks=exportPaths.reduce((result,route,i)=>{const worker=i%threads;if(!result[worker]){result[worker]={paths:[],pathMap:{}};}result[worker].pathMap[route]=exportPathMap[route];result[worker].paths.push(route);return result;},[]);const ampValidations={};let hadValidationError=false;const publicDir=(0,_path.join)(dir,_constants.CLIENT_PUBLIC_FILES_PATH);// Copy public directory
if((0,_fs.existsSync)(publicDir)){log('  copying "public" directory');await(0,_recursiveCopy.recursiveCopy)(publicDir,outDir,{filter(path){// Exclude paths used by pages
return!exportPathMap[path];}});}const workers=new Set();await Promise.all(chunks.map(chunk=>new Promise((resolve,reject)=>{const worker=(0,_child_process.fork)(require.resolve('./worker'),[],{env:process.env});workers.add(worker);worker.send({distDir,buildId,exportPaths:chunk.paths,exportPathMap:chunk.pathMap,outDir,renderOpts,serverRuntimeConfig,concurrency,subFolders,serverless:nextConfig.target==='serverless'});worker.on('message',({type,payload})=>{if(type==='progress'&&progress){progress();}else if(type==='error'){reject(payload);}else if(type==='done'){resolve();}else if(type==='amp-validation'){ampValidations[payload.page]=payload.result;hadValidationError=hadValidationError||payload.result.errors.length;}});})));workers.forEach(worker=>worker.kill());if(Object.keys(ampValidations).length){console.log((0,_index.formatAmpMessages)(ampValidations));}if(hadValidationError){throw new Error(`AMP Validation caused the export to fail. https://err.sh/zeit/next.js/amp-export-validation`);}// Add an empty line to the console for the better readability.
log('');}