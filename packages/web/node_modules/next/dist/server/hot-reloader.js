...
onDemandEntries.middleware(),webpackHotMiddleware,(0,_errorOverlayMiddleware.default)({dir:this.dir})];}async prepareBuildTools(multiCompiler){const tsConfigPath=(0,_path.join)(this.dir,'tsconfig.json');const useTypeScript=await fileExists(tsConfigPath);(0,_output.watchCompilers)(multiCompiler.compilers[0],multiCompiler.compilers[1],useTypeScript,({errors,warnings})=>this.send('typeChecked',{errors,warnings}));// This plugin watches for changes to _document.js and notifies the client side that it should reload the page
